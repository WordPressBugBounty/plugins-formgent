...
namespace FormGent\Stripe\ApiOperations;
...
trait Request
{
    ...
    protected static function _validateParams($params = null)
    {
        if ($params && !\is_array($params)) {
            ...
            throw new \FormGent\Stripe\Exception\InvalidArgumentException($message);
        }
    }
    /**
    ...
    protected static function _requestPage($url, $resultClass, $params = null, $options = null, $usage = [])
    {
        ...
        $obj = \FormGent\Stripe\Util\Util::convertToStripeObject($response->json, $opts);
        if (!$obj instanceof $resultClass) {
            throw new \FormGent\Stripe\Exception\UnexpectedValueException('Expected type ' . $resultClass . ', got "' . \get_class($obj) . '" instead.');
        }
    ...
    {
        $opts = \FormGent\Stripe\Util\RequestOptions::parse($options);
        $baseUrl = isset($opts->apiBase) ? $opts->apiBase : static::baseUrl();
        $requestor = new \FormGent\Stripe\ApiRequestor($opts->apiKey, $baseUrl);
