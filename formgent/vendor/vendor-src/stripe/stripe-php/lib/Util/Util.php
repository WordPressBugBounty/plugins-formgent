...
use FormGent\Stripe\StripeObject;
abstract class Util
{
            ...
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                if ('v2' === $apiMode && 'v2.core.event' === $resp['object']) {
                    ...
                    if (\array_key_exists('type', $resp) && \array_key_exists($resp['type'], $eventTypes)) {
                        ...
                        $class = \FormGent\Stripe\V2\Event::class;
                    }
                }
                ...
                $class = \FormGent\Stripe\V2\Collection::class;
            } else {
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \FormGent\Stripe\RelatedObject();
                    ...
                    $reason = new \FormGent\Stripe\Reason();
    ...
    {
        if ($h instanceof \FormGent\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
