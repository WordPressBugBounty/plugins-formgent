...
class MockResponse
{
     ...
     */
    public function __construct($body, int $status = 200, string $resourceKey = '')
    {
    ...
    {
        $psrResponse = $this->factories()->responseFactory->createResponse($this->status);
        $body = $this->factories()->streamFactory->createStream($this->body());
    ...
    {
        return ['body' => $this->body(), 'status' => $this->json()['status'] ?? 200, 'resourceKey' => $this->json()['resource_key'] ?? ''];
    }
