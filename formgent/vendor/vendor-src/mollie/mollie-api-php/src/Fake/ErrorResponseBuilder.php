...
class ErrorResponseBuilder
{
    ...
    public function __construct(int $status, string $title, string $detail, ?string $field = null)
    {
        $this->status = $status;
        $this->title = $title;
    ...
    public function create() : MockResponse
    {
        ...
        $contents = \str_replace(['{{ CODE }}', '{{ TITLE }}', '{{ DETAIL }}', '{{ FIELD }}'], [(string) $this->status, $this->title, $this->detail, $this->field], $contents);
