...
use FormGent\Mollie\Api\Traits\ForwardsCalls;
...
class ResourceResponseBuilder
{
    ...
    public function create() : MockResponse
    {
        ...
        $sampleContents = \json_decode(FakeResponseLoader::load($baseName), \true);
        $data = \array_merge(['_embedded' => []], ['_links' => $sampleContents['_links']], $this->data);
        ...
        $data = \array_merge($this->data, ['_embedded' => $data['_embedded'], '_links' => $data['_links']]);
