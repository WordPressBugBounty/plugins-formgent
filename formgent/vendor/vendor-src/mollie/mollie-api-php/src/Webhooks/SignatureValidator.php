...
use FormGent\Mollie\Api\Exceptions\InvalidSignatureException;
...
class SignatureValidator
{
    ...
    {
        $validSignatureFound = DataCollection::collect($signatures)->contains(fn($signatureHeader) => $this->isValidSignature($this->extractSignature($signatureHeader), $payload));
    ...
    {
        return \strpos($signatureHeader, self::SIGNATURE_PREFIX) === 0 ? \substr($signatureHeader, \strlen(self::SIGNATURE_PREFIX)) : $signatureHeader;
    }
    /**
    ...
    {
        return DataCollection::collect($this->signingSecrets)->contains(function ($secret) use($payload, $providedSignature) {
