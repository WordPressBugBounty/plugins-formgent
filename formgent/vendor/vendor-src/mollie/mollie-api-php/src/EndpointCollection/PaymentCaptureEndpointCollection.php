...
use FormGent\Mollie\Api\Factories\CreatePaymentCaptureRequestFactory;
use FormGent\Mollie\Api\Factories\GetPaginatedPaymentCapturesRequestFactory;
use FormGent\Mollie\Api\Factories\GetPaymentCaptureRequestFactory;
...
class PaymentCaptureEndpointCollection extends EndpointCollection
{
    ...
    public function createForId(string $paymentId, array $payload = [], bool $testmode = \false) : Capture
    {
        ...
        $request = CreatePaymentCaptureRequestFactory::new($paymentId)->withPayload($payload)->create();
        /** @var Capture */
    ...
    public function getForId(string $paymentId, string $captureId, array $query = [], bool $testmode = \false) : Capture
    {
        ...
        $request = GetPaymentCaptureRequestFactory::new($paymentId, $captureId)->withQuery($query)->create();
        /** @var Capture */
        return $this->send($request->test($testmode));
    }
    /**
    ...
    {
        $testmode = Utility::extractBool($query, 'testmode', $testmode);
        $request = GetPaginatedPaymentCapturesRequestFactory::new($paymentId)->withQuery($query)->create();
    ...
    public function iteratorForId(string $paymentId, ?string $from = null, ?int $limit = null, array $filters = [], bool $iterateBackwards = \false) : LazyCollection
    {
        ...
        $request = GetPaginatedPaymentCapturesRequestFactory::new($paymentId)->withQuery(['from' => $from, 'limit' => $limit, 'filters' => $filters])->create();
