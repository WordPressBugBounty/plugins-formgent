...
use FormGent\Mollie\Api\Factories\PaginatedQueryFactory;
use FormGent\Mollie\Api\Http\Requests\GetPaginatedBalanceTransactionRequest;
use FormGent\Mollie\Api\Resources\Balance;
use FormGent\Mollie\Api\Resources\BalanceTransactionCollection;
...
class BalanceTransactionEndpointCollection extends EndpointCollection
{
    ...
    public function pageForId(string $balanceId, array $query = [], bool $testmode = \false) : BalanceTransactionCollection
    {
        ...
        $query = PaginatedQueryFactory::new()->withQuery($query)->create();
        /** @var BalanceTransactionCollection */
        return $this->send((new GetPaginatedBalanceTransactionRequest($balanceId, $query->from, $query->limit))->test($testmode));
    }
    /**
    ...
    public function iteratorForId(string $balanceId, array $query = [], bool $iterateBackwards = \false, bool $testmode = \false) : LazyCollection
    {
        $testmode = Utility::extractBool($query, 'testmode', $testmode);
        $query = PaginatedQueryFactory::new()->withQuery($query)->create();
        return $this->send((new GetPaginatedBalanceTransactionRequest($balanceId, $query->from, $query->limit))->useIterator()->setIterationDirection($iterateBackwards)->test($testmode));
    }
}
