...
use FormGent\Mollie\Api\Factories\CreateProfileRequestFactory;
use FormGent\Mollie\Api\Factories\UpdateProfileRequestFactory;
use FormGent\Mollie\Api\Http\Requests\DeleteProfileRequest;
use FormGent\Mollie\Api\Http\Requests\GetCurrentProfileRequest;
use FormGent\Mollie\Api\Http\Requests\GetPaginatedProfilesRequest;
use FormGent\Mollie\Api\Http\Requests\GetProfileRequest;
...
class ProfileEndpointCollection extends EndpointCollection
{
    ...
    {
        $request = CreateProfileRequestFactory::new()->withPayload($payload)->create();
        /** @var Profile */
    ...
    {
        $request = UpdateProfileRequestFactory::new($profileId)->withPayload($payload)->create();
        /** @var Profile|null */
        return $this->send($request);
    }
    /**
    ...
    {
        return $this->send((new GetPaginatedProfilesRequest($from, $limit))->useIterator()->setIterationDirection($iterateBackwards));
    }
}
