...
use FormGent\Mollie\Api\Factories\CreatePaymentRefundRequestFactory;
use FormGent\Mollie\Api\Factories\CreatePaymentRequestFactory;
use FormGent\Mollie\Api\Factories\GetPaymentRequestFactory;
use FormGent\Mollie\Api\Factories\SortablePaginatedQueryFactory;
use FormGent\Mollie\Api\Factories\UpdatePaymentRequestFactory;
use FormGent\Mollie\Api\Http\Requests\CancelPaymentRequest;
use FormGent\Mollie\Api\Http\Requests\GetPaginatedPaymentsRequest;
use FormGent\Mollie\Api\Http\Requests\ReleasePaymentAuthorizationRequest;
...
class PaymentEndpointCollection extends EndpointCollection
{
    ...
    public function get(string $id, array $query = [], bool $testmode = \false) : Payment
    {
        ...
        $request = GetPaymentRequestFactory::new($id)->withQuery($query)->create();
    ...
    {
        $testmode = Utility::extractBool($query, 'testmode', $testmode);
        $request = CreatePaymentRequestFactory::new()->withPayload($payload)->withQuery($query)->create();
        /** @var Payment */
        return $this->send($request->test($testmode));
    }
    /**
    ...
    public function update(string $id, array $data = [], bool $testmode = \false) : ?Payment
    {
        ...
        $request = UpdatePaymentRequestFactory::new($id)->withPayload($data)->create();
        /** @var null|Payment */
        return $this->send($request->test($testmode));
    }
    /**
    ...
    public function refund(Payment $payment, array $payload = [], bool $testmode = \false) : Refund
    {
        ...
        $request = CreatePaymentRefundRequestFactory::new($payment->id)->withPayload($payload)->create();
        return $this->send($request->test($testmode));
    }
    /**
    ...
    public function page(?string $from = null, ?int $limit = null, array $filters = []) : PaymentCollection
    {
        ...
        return $this->send((new GetPaginatedPaymentsRequest($query->from, $query->limit, $query->sort))->test($testmode));
    }
    /**
    ...
    public function iterator(?string $from = null, ?int $limit = null, array $filters = [], bool $iterateBackwards = \false) : LazyCollection
    {
        $testmode = Utility::extractBool($filters, 'testmode', \false);
        $query = SortablePaginatedQueryFactory::new()->withQuery(['from' => $from, 'limit' => $limit, 'filters' => $filters])->create();
        return $this->send((new GetPaginatedPaymentsRequest($query->from, $query->limit, $query->sort))->useIterator()->setIterationDirection($iterateBackwards)->test($testmode));
    }
}
