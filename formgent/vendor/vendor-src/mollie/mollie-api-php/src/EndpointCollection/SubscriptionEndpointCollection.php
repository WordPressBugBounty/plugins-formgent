...
use FormGent\Mollie\Api\Factories\CreateSubscriptionRequestFactory;
use FormGent\Mollie\Api\Factories\GetAllPaginatedSubscriptionsRequestFactory;
use FormGent\Mollie\Api\Factories\UpdateSubscriptionRequestFactory;
use FormGent\Mollie\Api\Http\Requests\CancelSubscriptionRequest;
use FormGent\Mollie\Api\Http\Requests\GetPaginatedSubscriptionsRequest;
use FormGent\Mollie\Api\Http\Requests\GetSubscriptionRequest;
...
class SubscriptionEndpointCollection extends EndpointCollection
{
    ...
    public function createForId(string $customerId, array $payload = [], bool $testmode = \false) : Subscription
    {
        ...
        $request = CreateSubscriptionRequestFactory::new($customerId)->withPayload($payload)->create();
    ...
    {
        $testmode = Utility::extractBool($payload, 'testmode', $testmode);
        $request = UpdateSubscriptionRequestFactory::new($customerId, $subscriptionId)->withPayload($payload)->create();
        return $this->send($request->test($testmode));
    }
    /**
     ...
     */
    public function iteratorFor(Customer $customer, ?string $from = null, ?int $limit = null, array $filters = [], bool $iterateBackwards = \false) : LazyCollection
    {
     ...
     */
    public function iteratorForId(string $customerId, ?string $from = null, ?int $limit = null, array $filters = [], bool $iterateBackwards = \false) : LazyCollection
    {
        $testmode = Utility::extractBool($filters, 'testmode', \false);
        return $this->send((new GetPaginatedSubscriptionsRequest($customerId, $from, $limit))->useIterator()->setIterationDirection($iterateBackwards)->test($testmode));
    }
    ...
    public function allForId(?string $from = null, ?int $limit = null, array $filters = []) : SubscriptionCollection
    {
        $testmode = Utility::extractBool($filters, 'testmode', \false);
        $request = GetAllPaginatedSubscriptionsRequestFactory::new()->withQuery(['from' => $from, 'limit' => $limit, 'filters' => $filters])->create();
        return $this->send($request->test($testmode));
    }
    public function iteratorForAll(?string $from = null, ?int $limit = null, array $filters = [], bool $iterateBackwards = \true) : LazyCollection
    {
        $testmode = Utility::extractBool($filters, 'testmode', \false);
        $request = GetAllPaginatedSubscriptionsRequestFactory::new()->withQuery(['from' => $from, 'limit' => $limit, 'filters' => $filters])->create();
