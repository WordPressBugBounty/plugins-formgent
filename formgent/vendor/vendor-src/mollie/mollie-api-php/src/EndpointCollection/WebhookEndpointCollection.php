...
use FormGent\Mollie\Api\Factories\CreateWebhookRequestFactory;
use FormGent\Mollie\Api\Factories\UpdateWebhookRequestFactory;
use FormGent\Mollie\Api\Http\Requests\DeleteWebhookRequest;
use FormGent\Mollie\Api\Http\Requests\GetPaginatedWebhooksRequest;
use FormGent\Mollie\Api\Http\Requests\GetWebhookRequest;
use FormGent\Mollie\Api\Http\Requests\TestWebhookRequest;
...
class WebhookEndpointCollection extends EndpointCollection
{
    ...
    {
        $request = CreateWebhookRequestFactory::new()->withPayload($payload)->create();
        /** @var Webhook */
    ...
    {
        $request = UpdateWebhookRequestFactory::new($id)->withPayload($payload)->create();
        /** @var Webhook */
        return $this->send($request->test($test));
    }
    /**
     ...
     * @throws RequestException
     */
    public function page(?string $from = null, ?int $limit = null, ?string $sort = null, ?string $eventTypes = null) : WebhookCollection
    {
        $request = new GetPaginatedWebhooksRequest($from, $limit, $sort, $eventTypes);
     ...
     */
    public function iterator(?string $from = null, ?int $limit = null, ?string $sort = null, ?string $eventTypes = null, bool $iterateBackwards = \false, bool $test = \false) : LazyCollection
    {
        $request = new GetPaginatedWebhooksRequest($from, $limit, $sort, $eventTypes);
        return $this->send($request->useIterator()->setIterationDirection($iterateBackwards)->test($test));
    }
}
