...
use FormGent\Mollie\Api\Factories\GetPaginatedPaymentRefundsRequestFactory;
use FormGent\Mollie\Api\Factories\GetPaymentRefundRequestFactory;
use FormGent\Mollie\Api\Http\Requests\CancelPaymentRefundRequest;
...
class PaymentRefundEndpointCollection extends EndpointCollection
{
    ...
    {
        $testmode = Utility::extractBool($payload, 'testmode', \false) ?: Utility::extractBool($testmode, 'testmode', \false);
        $request = CreatePaymentRefundRequestFactory::new($paymentId)->withPayload($payload)->create();
    ...
    public function getForId(string $paymentId, string $refundId, array $query = [], bool $testmode = \false) : Refund
    {
        ...
        $request = GetPaymentRefundRequestFactory::new($paymentId, $refundId)->withQuery($query)->create();
        /** @var Refund */
        return $this->send($request->test($testmode));
    }
    /**
    ...
    public function pageForId(string $paymentId, ?string $from = null, ?int $limit = null, array $filters = []) : RefundCollection
    {
        ...
        $request = GetPaginatedPaymentRefundsRequestFactory::new($paymentId)->withQuery(['from' => $from, 'limit' => $limit, 'filters' => $filters])->create();
        return $this->send($request->test($testmode));
    }
    /**
     * @throws \Mollie\Api\Exceptions\RequestException
     ...
     * @param  bool  $iterateBackwards  Set to true for reverse order iteration (default is false).
     */
    public function iteratorForId(string $paymentId, ?string $from = null, ?int $limit = null, array $filters = [], bool $iterateBackwards = \false) : LazyCollection
    {
        $testmode = Utility::extractBool($filters, 'testmode', \false);
        $request = GetPaginatedPaymentRefundsRequestFactory::new($paymentId)->withQuery(['from' => $from, 'limit' => $limit, 'filters' => $filters])->create();
