...
use FormGent\Mollie\Api\Factories\GetClientRequestFactory;
use FormGent\Mollie\Api\Factories\GetPaginatedClientRequestFactory;
use FormGent\Mollie\Api\Resources\Client;
...
class ClientEndpointCollection extends EndpointCollection
{
    ...
    {
        $request = GetClientRequestFactory::new($id)->withQuery($query)->create();
        /** @var Client */
    ...
    {
        $request = GetPaginatedClientRequestFactory::new()->withQuery(['from' => $from, 'limit' => $limit, 'filters' => $filters])->create();
    ...
    {
        $request = GetPaginatedClientRequestFactory::new()->withQuery(['from' => $from, 'limit' => $limit, 'filters' => $filters])->create();
        return $this->send($request->useIterator()->setIterationDirection($iterateBackwards));
    }
}
