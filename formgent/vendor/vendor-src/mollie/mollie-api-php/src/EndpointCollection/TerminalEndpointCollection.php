...
use FormGent\Mollie\Api\Http\Requests\GetPaginatedTerminalsRequest;
use FormGent\Mollie\Api\Http\Requests\GetTerminalRequest;
use FormGent\Mollie\Api\Resources\LazyCollection;
use FormGent\Mollie\Api\Resources\Terminal;
use FormGent\Mollie\Api\Resources\TerminalCollection;
...
class TerminalEndpointCollection extends EndpointCollection
{
    ...
    {
        $testmode = Utility::extractBool($testmode, 'testmode', \false);
        return $this->send((new GetPaginatedTerminalsRequest($from, $limit))->useIterator()->setIterationDirection($iterateBackwards)->test($testmode));
    }
}
