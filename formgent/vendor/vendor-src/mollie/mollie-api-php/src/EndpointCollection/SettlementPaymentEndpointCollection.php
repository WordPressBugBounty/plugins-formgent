...
use FormGent\Mollie\Api\Http\Requests\GetPaginatedSettlementPaymentsRequest;
...
class SettlementPaymentEndpointCollection extends EndpointCollection
{
    ...
    public function pageForId(string $settlementId, array $query = [], bool $testmode = \false) : PaymentCollection
    {
        ...
        return $this->send((new GetPaginatedSettlementPaymentsRequest($settlementId, $query->from, $query->limit, $query->sort))->test($testmode));
    }
    /**
    ...
    public function iteratorForId(string $settlementId, ?string $from = null, ?int $limit = null, array $parameters = [], bool $iterateBackwards = \false) : LazyCollection
    {
        ...
        $query = SortablePaginatedQueryFactory::new(['from' => $from, 'limit' => $limit, 'filters' => $parameters])->create();
        return $this->send((new GetPaginatedSettlementPaymentsRequest($settlementId, $query->from, $query->limit, $query->sort))->useIterator()->setIterationDirection($iterateBackwards)->test($testmode));
    }
}
