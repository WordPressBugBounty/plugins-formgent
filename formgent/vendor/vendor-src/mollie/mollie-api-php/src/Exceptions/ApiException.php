...
class ApiException extends RequestException
{
     ...
     * @throws ApiException
     */
    public function __construct(Response $response, string $message, int $code, ?Throwable $previous = null)
    {
    ...
    public static function fromResponse(Response $response) : self
    {
        ...
        return new self($response, \sprintf('Error executing API call (%d: %s): %s', $status, $body->title ?? 'Unknown', $body->detail ?? 'Unknown'), $status, null);
    }
