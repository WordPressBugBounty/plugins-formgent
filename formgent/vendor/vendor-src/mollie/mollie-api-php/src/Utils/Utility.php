...
class Utility
{
    ...
    {
        $properties = \array_map(fn(ReflectionProperty $prop) => $prop->getName(), static::getProperties($class));
        return \array_filter($array, fn($key) => !\in_array($key, $properties, \true), \ARRAY_FILTER_USE_KEY);
    }
    /**
    ...
    {
        return \is_bool($value) ? $value : Arr::get($value, $key, $default);
    }
}
