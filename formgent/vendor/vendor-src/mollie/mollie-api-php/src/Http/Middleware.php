...
use FormGent\Mollie\Api\Contracts\ViableResponse;
use FormGent\Mollie\Api\Exceptions\MollieException;
use FormGent\Mollie\Api\Http\Middleware\Handlers;
...
class Middleware
{
    ...
    public function onResponse(callable $callback, ?string $name = null, string $priority = MiddlewarePriority::MEDIUM) : self
    {
        ...
        $this->onResponse->add(static function ($response) use($callback) {
            ...
            return $result instanceof Response || $result instanceof ViableResponse ? $result : $response;
        }, $name, $priority);
        return $this;
    }
    ...
    public function merge(...$handlersCollection) : self
    {
        ...
        foreach ($handlersCollection as $handlers) {
            $onRequestHandlers = \array_merge($this->onRequest->getHandlers(), $handlers->onRequest->getHandlers());
            $this->onRequest->setHandlers($onRequestHandlers);
            $onResponseHandlers = \array_merge($this->onResponse->getHandlers(), $handlers->onResponse->getHandlers());
            $this->onResponse->setHandlers($onResponseHandlers);
            $onFatalHandlers = \array_merge($this->onFatal->getHandlers(), $handlers->onFatal->getHandlers());
