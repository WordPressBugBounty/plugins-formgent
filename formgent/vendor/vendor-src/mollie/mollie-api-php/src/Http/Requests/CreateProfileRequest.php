...
class CreateProfileRequest extends ResourceHydratableRequest implements HasPayload
{
    ...
    public function __construct(string $name, string $website, string $email, string $phone, ?string $description = null, ?array $countriesOfActivity = null, ?string $businessCategory = null)
    {
        $this->name = $name;
    ...
    {
        return ['name' => $this->name, 'website' => $this->website, 'email' => $this->email, 'phone' => $this->phone, 'description' => $this->description, 'countriesOfActivity' => $this->countriesOfActivity, 'businessCategory' => $this->businessCategory];
    }
    /**
     * The resource path.
     */
