...
class CreateWebhookRequest extends ResourceHydratableRequest implements HasPayload, SupportsTestmodeInPayload
{
     ...
     */
    private $eventTypes;
    public function __construct(string $url, string $name, $eventTypes)
    {
        $this->url = $url;
        $this->name = $name;
    ...
    {
        return ['url' => $this->url, 'name' => $this->name, 'eventTypes' => \is_string($this->eventTypes) ? $this->eventTypes : Arr::join($this->eventTypes)];
    }
    /**
     * The resource path.
     */
