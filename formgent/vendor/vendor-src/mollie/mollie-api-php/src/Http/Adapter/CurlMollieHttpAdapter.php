...
final class CurlMollieHttpAdapter implements HttpAdapterContract
{
    ...
    protected function send(PendingRequest $pendingRequest) : array
    {
        ...
        try {
            $curl = CurlFactory::new($request->getUri(), $pendingRequest)->withHeaders($pendingRequest->headers()->all())->withMethod($pendingRequest->method(), $request->getBody())->create();
    ...
    protected function createResponse(PendingRequest $pendingRequest, int $statusCode, $headers = [], $body = null, ?Throwable $error = null) : Response
    {
        ...
        $response = $responseFactory->createResponse($statusCode)->withBody($factoryCollection->streamFactory->createStream($body));
        foreach ($headers as $key => $value) {
            $response = $response->withHeader($key, $value);
        }
        return new Response($response, $pendingRequest->createPsrRequest(), $pendingRequest, $error);
    }
    /**
     * @return array{0: array<string, string>, 1: string, 2: int}
     */
