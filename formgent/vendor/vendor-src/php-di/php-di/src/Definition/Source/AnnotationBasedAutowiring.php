...
namespace FormGent\DI\Definition\Source;

use FormGent\DI\Annotation\Inject;
use FormGent\DI\Annotation\Injectable;
use FormGent\DI\Definition\Exception\InvalidAnnotation;
use FormGent\DI\Definition\ObjectDefinition;
use FormGent\DI\Definition\ObjectDefinition\MethodInjection;
use FormGent\DI\Definition\ObjectDefinition\PropertyInjection;
use FormGent\DI\Definition\Reference;
use FormGent\Doctrine\Common\Annotations\AnnotationRegistry;
use FormGent\Doctrine\Common\Annotations\Reader;
use FormGent\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use FormGent\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'FormGent\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'FormGent\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('FormGent\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'FormGent\\DI\\Annotation\\Injectable');
