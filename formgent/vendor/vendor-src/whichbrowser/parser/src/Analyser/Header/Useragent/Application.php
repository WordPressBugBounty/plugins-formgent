...
namespace FormGent\WhichBrowser\Analyser\Header\Useragent;

use FormGent\WhichBrowser\Constants;
use FormGent\WhichBrowser\Data;
use FormGent\WhichBrowser\Model\Family;
use FormGent\WhichBrowser\Model\Using;
...
trait Application
{
    ...
    private function detectSpecificApplications($ua)
    {
        ...
        if (\preg_match('/Android [0-9\\.]+; (.*) Sony\\/.*SonySelectSDK\\/([0-9\\.]+)/iu', $ua, $match)) {
            ...
            $this->data->browser->type = Constants\BrowserType::APP;
            $this->data->browser->using = new \FormGent\WhichBrowser\Model\Using(['name' => 'Sony Select SDK', 'version' => new Version(['value' => $match[2], 'details' => 2])]);
            $this->data->device->model = $match[1];
            $this->data->device->identified |= Constants\Id::PATTERN;
            $this->data->device->type = Constants\DeviceType::MOBILE;
            $device = Data\DeviceModels::identify('android', $match[1]);
