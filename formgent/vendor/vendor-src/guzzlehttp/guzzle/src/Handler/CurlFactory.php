...
namespace FormGent\GuzzleHttp\Handler;

use FormGent\GuzzleHttp\Exception\ConnectException;
use FormGent\GuzzleHttp\Exception\RequestException;
use FormGent\GuzzleHttp\Promise as P;
use FormGent\GuzzleHttp\Promise\FulfilledPromise;
use FormGent\GuzzleHttp\Promise\PromiseInterface;
use FormGent\GuzzleHttp\Psr7\LazyOpenStream;
use FormGent\GuzzleHttp\TransferStats;
use FormGent\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \FormGent\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \FormGent\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \FormGent\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \FormGent\GuzzleHttp\Psr7\Utils::streamFor($sink);
