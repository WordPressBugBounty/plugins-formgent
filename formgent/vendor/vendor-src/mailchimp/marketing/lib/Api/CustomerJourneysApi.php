...
class CustomerJourneysApi
{
    ...
    protected function triggerRequest($journey_id, $step_id, $body)
    {
        ...
        if ($journey_id === null || \is_array($journey_id) && \count($journey_id) === 0) {
            throw new \InvalidArgumentException('Missing the required parameter $journey_id when calling ');
        }
        // verify the required parameter 'step_id' is set
        if ($step_id === null || \is_array($step_id) && \count($step_id) === 0) {
            throw new \InvalidArgumentException('Missing the required parameter $step_id when calling ');
        }
        ...
        // path params
        if ($journey_id !== null) {
            $resourcePath = \str_replace('{' . 'journey_id' . '}', ObjectSerializer::toPathValue($journey_id), $resourcePath);
        }
        // path params
        if ($step_id !== null) {
            $resourcePath = \str_replace('{' . 'step_id' . '}', ObjectSerializer::toPathValue($step_id), $resourcePath);
        }
        // body params
        $_tempBody = null;
        if (isset($body)) {
