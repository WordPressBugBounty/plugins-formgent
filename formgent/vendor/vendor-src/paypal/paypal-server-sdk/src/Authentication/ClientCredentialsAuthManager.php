 ...
 */
namespace FormGent\PaypalServerSdkLib\Authentication;

use Closure;
use Exception;
use FormGent\CoreInterfaces\Core\Request\TypeValidatorInterface;
use FormGent\Core\Authentication\CoreAuth;
...
use FormGent\PaypalServerSdkLib\ConfigurationDefaults;
use FormGent\PaypalServerSdkLib\ClientCredentialsAuth;
/**
...
class ClientCredentialsAuthManager extends CoreAuth implements ClientCredentialsAuth
{
    ...
    {
        return $oAuthClientId == $this->getOAuthClientId() && $oAuthClientSecret == $this->getOAuthClientSecret();
    }
    /**
    ...
    public function fetchToken(?array $additionalParams = null) : OAuthToken
    {
        ...
        $response = $this->oAuthApi->requestToken(['authorization' => $this->buildBasicHeader(), 'scope' => null], $additionalParams);
        if ($response->isError()) {
    ...
    public function isTokenExpired(?OAuthToken $token = null) : bool
    {
        ...
        return $token->getExpiry() < \time() + $this->getOAuthClockSkew();
    }
    ...
    public function validate(TypeValidatorInterface $validator) : void
    {
        ...
        parent::__construct(HeaderParam::init('Authorization', CoreHelper::getBearerAuthString($this->internalOAuthToken->getAccessToken()))->requiredNonEmpty());
    ...
    {
        return 'Basic ' . \base64_encode($this->getOAuthClientId() . ':' . $this->getOAuthClientSecret());
    }
    /**
    ...
    private function addExpiryTime(OAuthToken $oAuthToken) : void
    {
        ...
        $oAuthToken->setExpiry(\time() + $expiresIn);
    }
}
